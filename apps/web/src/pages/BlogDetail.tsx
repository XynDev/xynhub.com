...
export function BlogDetail() {
  ...
  return (
    <main className="pt-48 pb-20 px-6 md:px-8 max-w-[1440px] w-full mx-auto">
      <SEO title={blog.title} description={blog.category} />
        ...
        <article className="space-y-12">
          <section className="prose prose-invert max-w-none">
            ...
            <p className="text-on-surface leading-loose mb-8 text-lg text-on-surface-variant">
              {content.dilemma?.paragraph}
            </p>
          </section>

          {/* Bento High-Contrast Diagram 1 */}
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <BentoCard className="p-5 md:p-10 flex flex-col justify-between border border-outline-variant/30 bg-surface-container">
              <div className="mb-8">
                <span className="material-symbols-outlined text-primary text-2xl md:text-4xl mb-4">cloud_off</span>
                <h3 className="text-xl font-bold text-primary">{content.diagram1?.centralized?.title}</h3>
                <p className="text-sm text-on-surface-variant mt-2 leading-relaxed">
                  {content.diagram1?.centralized?.description}
                </p>
              </div>
            ...
            <BentoCard className="p-5 md:p-10 flex flex-col justify-between border border-outline-variant/30 bg-surface-container-high">
              <div className="mb-8">
                <span className="material-symbols-outlined text-primary text-2xl md:text-4xl mb-4">dynamic_form</span>
                <h3 className="text-xl font-bold text-primary">{content.diagram1?.edge?.title}</h3>
                <p className="text-sm text-on-surface-variant mt-2 leading-relaxed">
                  {content.diagram1?.edge?.description}
                </p>
              </div>
              <div className="h-32 bg-surface-container-low rounded-2xl flex items-end px-6 py-4 gap-2">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <BentoCard className="md:col-span-2 p-6 md:p-12 border border-outline-variant/30 bg-surface-container-high">
              <h3 className="label-sm text-on-surface-variant tracking-widest uppercase mb-4">{content.diagram2?.stack?.title}</h3>
              <div className="space-y-6">
                {content.diagram2?.stack?.items?.map((item: AnyData) => (
            ...
            <BentoCard className="p-0 overflow-hidden relative border border-outline-variant/30 h-64 md:h-auto">
              <img alt="Hardware" className="w-full h-full object-cover opacity-50 grayscale" src={content.diagram2?.hardware?.image} />
              <div className="absolute inset-0 bg-gradient-to-t from-surface-container to-transparent p-5 md:p-8 flex flex-col justify-end">
                <p className="text-primary font-bold">{content.diagram2?.hardware?.title}</p>
                <p className="label-sm text-on-surface-variant">{content.diagram2?.hardware?.label}</p>
              </div>
            </BentoCard>
          </div>
